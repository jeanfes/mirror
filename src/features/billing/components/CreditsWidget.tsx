...
export function CreditsWidget() {
  ...
  const lastGenerationAtStr = account.lastGenerationAt
    ? format(new Date(account.lastGenerationAt), "dd/MM HH:mm")
    : ""

  return (
    <div className="hidden sm:flex flex-col justify-center min-w-35 px-3 py-1.5 rounded-2xl bg-surface-base/50 border border-border-soft">
      ...
      />
      {lastGenerationAtStr ? (
        <p className="mt-1 text-[10px] text-secondary-text truncate">
          {t.app.account.latestOutput}: {lastGenerationAtStr}
        </p>
      ) : null}
    </div>
  )
