...
export function MobileSidebar() {
  ...
  const user = useMemo(
    () => ({
      name: storeUser?.name ?? storeUser?.email?.split("@")[0] ?? "User",
            ...
            return (
              <Link
                ...
                className={clsx(
                  ...
                  isActive
                    ? "text-white"
                    : "text-secondary-text hover:bg-surface-hover hover:text-primary-dark"
                )}
              >
                {isActive && (
                  <div
  ...
  return (
    <div className="block md:hidden">
      ...
      {typeof window !== "undefined"
        ? createPortal(sidebarOverlay, document.body)
        : null}
